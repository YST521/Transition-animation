...
-(void)setUI
{
    ...
    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(300, [UIScreen mainScreen].bounds.size.height-60, 50, 60)];
    _imageView.image = [UIImage imageNamed:@"TabCartSelected@2x.png"];
//    _imageView.center = CGPointMake(270, 320);
    [self.view addSubview:_imageView];
    // label
    _cntLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_imageView.frame)-15, CGRectGetMinY(_imageView.frame)+5, 20, 20)];
    ...
    //起点
    [_path moveToPoint:CGPointMake(50, 350)];
//    终点
    [_path addQuadCurveToPoint:CGPointMake(300,  [UIScreen mainScreen].bounds.size.height-60) controlPoint:CGPointMake(150, 20)];
}
-(void)startAnimation
{
    if (!layer) {
